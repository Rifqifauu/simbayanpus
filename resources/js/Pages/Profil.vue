<template>
    <div class="min-h-screen bg-gray-50 py-8 px-4">
        ...
        >
            <!-- Form Header -->
            <div
                class="bg-red-600 text-white flex justify-between items-center p-4 rounded-2xl"
            >
            ...
            <form @submit.prevent="handleSubmit" class="md:flex">
                ...
                <aside class="md:w-2/5 p-6 bg-gray-50">
                    ...
                    <div class="text-center mb-6">
                        <div
                            class="w-60 h-80 mx-auto rounded overflow-hidden border-4 border-white shadow-lg relative group"
                        >
                            <img
                            ...
                            <div
                                v-else
                                class="w-full h-full p-8 bg-gray-200 flex items-center justify-center"
                            >
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                            ...
                            >
                                <label
                                    class="cursor-pointer bg-white text-gray-800 px-4 py-2 rounded-lg hover:bg-gray-100"
                                >
                                    <input
                                        type="file"
                                        class="hidden"
    ...
    >
        <div class="bg-white p-6 rounded-lg shadow-lg w-80">
            <div class="flex justify-between items-center">
                <h3 class="text-xl font-semibold text-blue-700">Konfirmasi</h3>
            </div>
            <div class="mt-4">
                <p class="text-gray-800">
                    Apakah Anda yakin ingin melanjutkan pengeditan ini?
                </p>
            </div>
    ...
    setup(props) {
        ...
        const handleSubmit = async () => {
            try {
                ...
                const endpoint = props.userDetail
                    ? "/profile/edit"
                    : "/profile/store";
                ...
                if (error.response && error.response.data.errors) {
                    ...
                    for (let key in error.response.data.messages) {
                        if (error.response.data.messages.hasOwnProperty(key)) {
                            ...
                            console.log(
                                `Error in ${key}:`,
                                error.response.data.messages[key][0]
                            );
                        }
                    }
                }
